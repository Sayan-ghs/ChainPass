...
const NetworkBanner = () => {
  ...
  // Only show the banner if connected to Base Sepolia
  const isBaseSepolia = chain?.id === baseSepolia.id;
  
  // Log network information to console
  useEffect(() => {
    if (isConnected && chain) {
      console.log('Connected to network:', {
        name: chain.name,
        id: chain.id,
        isBase: chain.id === base.id || chain.id === baseSepolia.id,
        isBaseSepolia: chain.id === baseSepolia.id
      });
    }
  }, [chain, isConnected]);

  // Hide after clicking dismiss
  const handleDismiss = () => {
    setAnimateOut(true);
    setTimeout(() => setVisible(false), 300);
  };

  if (!visible || !isConnected || !isBaseSepolia) {
    return null;
  }

  return (
    <div className={`bg-blue-50 transition-all duration-300 ${animateOut ? 'opacity-0 -translate-y-4' : 'opacity-100 animate-slideDown'}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <p className="text-sm font-medium text-blue-700">
              You're connected to <span className="font-bold">Base Sepolia Testnet</span> (Chain ID: {chain?.id})
            </p>
          </div>
          <button 
